...
export async function instantSearchProfiles(q: string): Promise<any[]> {
  try {
    const results = await client.collections('profiles').documents().search({
      q:         q || '*',
...
export async function instantSearchPosts(q: string): Promise<any[]> {
  try {
