...
import { Compass, Home, MessageCircle, Settings2, User } from "lucide-react";
...
export default function LeftSidebar() {
  ...
  const { data: stats } = useGetProfile(profile?.username);

  if (!profile || !user) return null;

  const navLinks = [
    { href: dashboardPathForRole(role), label: "Home", icon: Home },
    ...
    <div className="bg-white rounded-2xl border border-[color:var(--line)] shadow-[0_2px_8px_-4px_rgba(40,20,5,0.1)] overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="relative h-24 overflow-hidden">
          {stats?.bannerUrl ? (
            ...
            <>
              <div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, #9a3412 0%, #c2410c 40%, #f97316 75%, #fdba74 100%)",
                }}
              />
              <div
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(0deg, rgba(28,20,16,0.15), transparent)",
            }}
          />
        </div>
          ...
          <Link href={`/profile/${profile.username}`}>
            <div className="w-16 h-16 rounded-full bg-orange-100 flex items-center justify-center text-[color:var(--orange-700)] font-bold text-2xl overflow-hidden ring-4 ring-white shadow-[0_4px_14px_rgba(40,20,5,0.15)] hover:scale-105 hover:ring-orange-100 transition-all duration-300">
              {(stats?.avatarUrl ?? profile.avatarUrl) ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={stats?.avatarUrl ?? profile.avatarUrl}
      ...
      <div className="px-4 pt-12 pb-4">
        <Link href={`/profile/${profile.username}`} className="group block">
          ...
          {profile.location && (
            <p className="text-[11px] text-[color:var(--ink-soft)] mt-1.5">
              {profile.location}
            </p>
          )}
        </Link>
      ...
      <div className="border-t border-[color:var(--line)] grid grid-cols-3 text-center">
        {[
          ...
          <div
            key={label}
            className={`py-3.5 ${i > 0 ? "border-l border-[color:var(--line)]" : ""}`}
          >
            <p className="font-playfair text-[18px] font-bold text-[color:var(--ink)] leading-none">
              {Number(value).toLocaleString()}
            </p>
            <p className="font-dm-sans text-[10px] text-gray-400 mt-1.5 uppercase tracking-wider">
              {label}
            </p>
          </div>
        ))}
      ...
      <nav className="p-2 space-y-0.5 border-t border-[color:var(--line)]">
        {navLinks.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="absolute left-0 top-0 bottom-0 w-[4px] rounded-r-full bg-primary" />
              )}
              <Icon
                className={`w-[17px] h-[17px] shrink-0 ${active ? "stroke-[2.4]" : "stroke-[1.8]"}`}
              />
              {label}
            </Link>
          );
