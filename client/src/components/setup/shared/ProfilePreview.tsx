...
export default function ProfilePreview({ state, editMode, onField, onServiceEdit }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-100 max-w-sm w-full mx-auto">
      {/* Hero */}
      <div className="relative pb-12" style={{ backgroundColor: color }}>
        <div className="pt-8 px-6 pb-0 text-center">
          {state.photos[0] ? (
            ...
            <img
              src={state.photos[0]}
              alt=""
              className="w-20 h-20 rounded-full mx-auto border-3 border-white object-cover"
              style={{ border: "3px solid white" }}
            />
          ) : (
            <div
              className="w-20 h-20 rounded-full mx-auto flex items-center justify-center text-white text-2xl font-bold font-playfair"
              style={{ backgroundColor: "rgba(255,255,255,0.25)", border: "3px solid white" }}
            >
              {initials}
            </div>
          )}
          ...
          <div className="mt-3 text-white">
            {editMode && edit("fullName") ? (
              <EditableField
                ...
                className="text-xl font-bold font-playfair text-white block"
              />
            ) : (
            ...
            <div className="flex items-center justify-center gap-1 mt-0.5 text-white/80 text-sm">
              {editMode && edit("profession") ? (
                <EditableField value={state.profession} onSave={edit("profession")!} placeholder="Profession" className="text-white/80" />
              ) : (
                <span>{state.profession || <Placeholder text="Profession" />}</span>
              )}
              {(state.profession || state.location) && <span>·</span>}
              {editMode && edit("location") ? (
                <EditableField value={state.location} onSave={edit("location")!} placeholder="Location" className="text-white/80" />
              ) : (
                <span>{state.location || <Placeholder text="Location" />}</span>
              )}
            </div>

            <div className="mt-1.5 text-white/90 text-sm italic">
              {editMode && edit("tagline") ? (
                <EditableField value={state.tagline} onSave={edit("tagline")!} placeholder="Your tagline" className="text-white/90 italic" />
              ) : (
      ...
      <div className="px-4 pt-4 pb-2">
        ...
        {state.yearsExperience > 0 && (
          <span
            ...
            style={{ backgroundColor: color }}
          >
            {state.yearsExperience} years experience
          </span>
        )}
      </div>
