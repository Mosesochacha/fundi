...
function strengthLabel(p: string): { label: string; color: string; width: string } {
  ...
  if (!/[A-Z]/.test(p) || !/[0-9]/.test(p)) return { label: "Fair", color: "bg-yellow-400", width: "60%" };
...
function parseAgent(ua: string) {
  ...
  const browser = ua.match(/(Chrome|Firefox|Safari|Edge|Opera|OPR)\//i)?.[1] || "Browser";
    ...
    <div className="space-y-6">
      ...
      <SettingsSection title="Password">
        <div className="py-4 space-y-4">
          ...
          <Button onClick={handlePasswordChange} loading={changingPwd} variant="primary">
            Update password
          </Button>
        </div>
      </SettingsSection>
      ...
      <SettingsSection title="Active Sessions">
        <div className="py-2">
          {sessions.length === 0 && (
            <p className="text-sm text-gray-400 py-4 text-center">No active sessions found</p>
          )}
          {sessions.map((s: any, i: number) => {
            const { device, icon: DeviceIcon } = parseAgent(s.userAgent || "");
            return (
              <div key={s.id} className={`flex items-center gap-3 py-3.5 ${i < sessions.length - 1 ? "border-b border-gray-100" : ""}`}>
                <DeviceIcon size={18} className="text-gray-400 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{device}</p>
                  <p className="text-xs text-gray-400">{s.ipAddress || "Unknown IP"} · {timeAgo(s.createdAt)}</p>
                </div>
                <button
      ...
      <SettingsSection title="Login History">
        <div className="py-2">
          {history.length === 0 && (
            <p className="text-sm text-gray-400 py-4 text-center">No login history</p>
          )}
          {history.map((h: any, i: number) => (
            <div key={h.id} className={`flex items-center gap-3 py-3 ${i < history.length - 1 ? "border-b border-gray-100" : ""}`}>
              {h.status === "success"
                ? <CheckCircle size={16} className="text-green-500 shrink-0" />
                : <XCircle size={16} className="text-red-400 shrink-0" />
              }
              <div className="flex-1 min-w-0">
                <p className="text-xs text-gray-700 truncate">{parseAgent(h.userAgent || "").device}</p>
                <p className="text-xs text-gray-400">{h.ipAddress || "Unknown"} · {timeAgo(h.createdAt)}</p>
              </div>
              <span className={`text-xs font-medium shrink-0 ${h.status === "success" ? "text-green-500" : "text-red-400"}`}>
      ...
      <SettingsSection title="Account Status">
        <SettingsRow
          ...
          description={user?.emailVerified ? "Your email is verified" : "Your email is not yet verified"}
          last
        >
          {user?.emailVerified
