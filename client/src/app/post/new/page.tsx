...
import { useCreatePost, usePolishPost } from "@/features/posts";
import { useGetProfilePosts } from "@/features/profiles";
...
export default function NewPostPage() {
  const { isLoggedIn, profile, role } = useAuth();
  ...
  const creating = createPostMutation.isPending;
  const polishPostMutation = usePolishPost();
  const polishing = polishPostMutation.isPending;

  // Fetch recent posts for the card below
  const { data: recentPostsData } = useGetProfilePosts(profile?.username, 1);
  const recentPosts: any[] = (recentPostsData as any)?.posts?.slice(0, 3) ?? [];
  ...
  if (!isLoggedIn) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <Link
          href="/login"
          className="bg-primary text-white px-5 py-2 rounded-xl text-sm font-medium hover:bg-orange-600 transition-colors"
        >
          Sign in
        </Link>
      </div>
  ...
  const handlePhotoFile = async (file: File) => {
    ...
    if (images.length >= 4) {
      warning("Maximum 4 photos per post");
      return;
    }
    ...
    try {
      ...
      const res = await client.post(`/photos/work`, formData);
      const data = res.data;
      if (!data.success) throw new Error(data.message);
      setImages((prev) =>
        prev.map((s) =>
          s.url === slot.url ? { url: data.data.url, uploading: false } : s,
        ),
      );
    } catch {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await polishPostMutation.mutateAsync({
        roughText: aiNotes,
        profession: profile?.profession,
        postType: "SHOWCASE",
      });
      const generated = (res.data as { data: { content: string } }).data
        .content;
      if (content.trim()) {
  ...
  const handleSchedule = () => {
    if (!schedDate || !schedTime) {
      warning("Pick both a date and time.");
      return;
    }
  ...
  const handleSubmit = async () => {
    if (!content.trim()) {
      warning("Write something first.", 4000);
      return;
    }
    if (images.some((i) => i.uploading)) {
      warning("Wait for photos to finish uploading.");
      return;
    }
    if (scheduledAt && new Date(scheduledAt) <= new Date()) {
      warning("Scheduled time must be in the future.", 4000);
      return;
    }
    ...
    try {
      ...
      const res = await createPostMutation.mutateAsync(payload);
      const data = (
        res.data as { data: { id: string; slug: string; status: string } }
      ).data;
  ...
  const audienceLabel =
    AUDIENCE_OPTIONS.find((o) => o.value === audience)?.label ?? "Anyone";
  ...
  const scheduledDisplay = scheduledAt
    ? new Date(scheduledAt).toLocaleString("en", {
        weekday: "short",
        day: "numeric",
        month: "short",
        hour: "2-digit",
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        {scheduledAt && (
          <div className="flex items-center justify-between bg-orange-50 border border-orange-200 rounded-xl px-4 py-2.5">
            ...
            <button
              ...
              className="text-xs text-gray-400 hover:text-gray-600 transition-colors font-dm-sans"
            >
              Remove
            </button>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="flex items-start gap-3 px-5 pt-4 pb-3 border-b border-gray-100">
            ...
            <div className="flex flex-col gap-1.5 pt-0.5">
              <p className="text-[15px] font-semibold text-gray-900 leading-tight font-dm-sans">
                {profile?.fullName}
              </p>
              {/* Audience selector */}
              <div className="relative">
                ...
                >
                  ...
                  <span className="text-[13px] text-gray-600 font-dm-sans">
                    {audienceLabel}
                  </span>
            ...
            <div className="px-5 pb-3">
              {images.length === 1 && (
                <div className="relative rounded-xl overflow-hidden h-[400px]">
                  ...
                  <button
                    onClick={() => setImages([])}
                    className="absolute top-2 right-2 w-6 h-6 bg-white rounded-full shadow flex items-center justify-center text-gray-700 hover:text-red-500 transition-colors"
                  >
              ...
              {images.length === 3 && (
                <div className="grid grid-cols-2 gap-2 h-[480px]">
                  <div className="relative rounded-xl overflow-hidden row-span-2">
                    ...
                    <button
                      onClick={() =>
                        setImages(images.filter((_, j) => j !== 0))
                      }
                      className="absolute top-2 right-2 w-6 h-6 bg-white rounded-full shadow flex items-center justify-center text-gray-700 hover:text-red-500 transition-colors"
                    >
                      <X className="w-3.5 h-3.5" />
                  ...
                  {images.slice(1).map((img, i) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          setImages(images.filter((_, j) => j !== i + 1))
                        }
                        className="absolute top-2 right-2 w-6 h-6 bg-white rounded-full shadow flex items-center justify-center text-gray-700 hover:text-red-500 transition-colors"
                      >
                        <X className="w-3.5 h-3.5" />
              ...
              {images.length === 4 && (
                <div className="grid grid-cols-2 gap-2">
                  {images.map((img, i) => (
                    <div
                      key={i}
                      className="relative rounded-xl overflow-hidden h-[200px]"
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={img.url}
              ...
              <div className="bg-gray-50 rounded-xl border border-gray-100 p-3 space-y-2">
                ...
                {showAiConfirm ? (
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] text-gray-600 font-dm-sans flex-1">
                      Replace your current text?
                    </span>
                    <button
                      onClick={() => confirmReplace(true)}
                      className="text-[13px] font-semibold text-white bg-primary px-3 py-1.5 rounded-lg hover:bg-orange-600 transition-colors"
                    >
                      Yes
                    </button>
                    <button
                      onClick={() => confirmReplace(false)}
                      className="text-[13px] font-semibold text-gray-600 bg-gray-100 px-3 py-1.5 rounded-lg hover:bg-gray-200 transition-colors"
                    >
                      No
                    </button>
                  </div>
          ...
          <div className="relative px-5 py-3 border-t border-gray-100 flex items-center justify-between">
                  ...
                  <div className="grid grid-cols-6 gap-1">
                    {EMOJIS.map((e) => (
                      <button
                        ...
                        className="w-9 h-9 text-xl flex items-center justify-center rounded-lg hover:bg-gray-100 transition-colors"
                      >
                        {e}
                      </button>
                    ))}
            ...
            {showSchedulePanel && (
              <>
                ...
                <div className="absolute bottom-full right-0 mb-2 z-20 bg-white border border-gray-200 rounded-2xl shadow-lg p-4 w-72">
                  <p className="text-[14px] font-semibold text-gray-900 font-dm-sans">
                    Schedule post
                  </p>
                  <p className="text-[13px] text-gray-400 mt-1 mb-3 font-dm-sans">
                  ...
                  <div className="flex gap-2 mt-3">
                    <button
                      ...
                      className="flex-1 h-9 bg-primary text-white text-sm font-semibold rounded-lg hover:bg-orange-600 transition-colors font-dm-sans"
                    >
                      Schedule
                    </button>
                    <button
                      onClick={() => setShowSchedulePanel(false)}
                      className="flex-1 h-9 bg-gray-100 text-gray-600 text-sm font-semibold rounded-lg hover:bg-gray-200 transition-colors font-dm-sans"
                    >
                      Cancel
                    </button>
                  </div>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                <Sparkles
                  className={`w-5 h-5 ${showAiPanel ? "text-orange-500" : "text-orange-400"}`}
                />
              </button>
              <button
                onClick={() => {
              ...
              >
                <Clock
                  className={`w-5 h-5 ${scheduledAt ? "text-orange-500" : "text-gray-400"}`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center gap-3">
              <span
                className={`text-[13px] tabular-nums font-dm-sans ${charCount > 3000 ? "text-red-500" : charCount > 2500 ? "text-orange-500" : "text-gray-400"}`}
              >
                {charCount} / 3000
              </span>
              <button
          ...
          <ul className="mt-3 space-y-2.5">
            {[
              ...
              <li key={i} className="flex items-start gap-2.5">
                ...
                <span className="text-[13px] text-gray-500 leading-[1.5] font-dm-sans">
                  {tip}
                </span>
              </li>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[13px] font-semibold text-gray-900 font-dm-sans">
              Your recent posts
            </p>
            {profile?.username && (
              <Link
                href={`/profile/${profile.username}`}
                className="text-xs text-primary hover:underline font-dm-sans"
              >
                View all →
              </Link>
            )}
          ...
          {recentPosts.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentPosts.map((p: any) => (
                ...
                >
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 mt-1.5 ${POST_DOT[p.postType] ?? "bg-gray-300"}`}
                  />
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-gray-400 mt-1 font-dm-sans">
                      {timeAgo(p.createdAt)} · {p.likesCount ?? 0} likes ·{" "}
